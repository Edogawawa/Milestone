...
import fetchTodos from "./service/network"
/**
 * @typedef {{
 *  id: string
 *  taskName: string
...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() { // Mark this function as async
      try {
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {toDos.filter(filterFn).sort(compareFn).map(toDo => <tr className="h-12" key={toDo.id}>
              ...
              <td>
                <div className="flex items-center">{toDo?.date ? toDo?.date : ""}</div>
              </td>

              <td>
                <div className="flex flex-row gap-2 text-sm whitespace-nowrap overflow-auto w-60">
                  {JSON.parse(toDo?.category)?.map(c => categoryFilter?.has(c) ? null : <button 
                    key={c}
